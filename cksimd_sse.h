...
namespace ck_simd {
	...
	void storeu(double *p, __m128d x, sse_double_tag) {
			_mm_storeu_pd(p, x);
	}
	__m128 max(__m128 x, __m128 y, sse_float_tag) {
		return _mm_max_ps(x, y);
	}
	__m128d max(__m128d x, __m128d y, sse_double_tag) {
		return _mm_max_pd(x, y);
	}
	__m128 min(__m128 x, __m128 y, sse_float_tag) {
		return _mm_min_ps(x, y);
	}
	__m128d min(__m128d x, __m128d y, sse_double_tag) {
